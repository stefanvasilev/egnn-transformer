...
class EGNNTransformer(nn.Module):
    ...
    def setup(self):
        ...
        self.input_layer_pos = nn.Dense(self.model_dim)
    ...
    def __call__(self, node_inputs, edge_inputs, coords, vel, cross_mask=None, train=True):
        ...
        if self.use_pos:
            # Input layer
            node_inputs, pos_encoding = node_inputs[:, :, :num_node_features-1], node_inputs[:, :, num_node_features-1:]
            node_inputs = node_inputs + self.input_layer_pos(pos_encoding)
        # Input layer
